...
export async function bindingStatus(env:Env){let d1=false,r2=false,queue=false;try{await env.DB.prepare("SELECT 1").first();d1=true}catch{}try{await env.MEDIA.list({limit:1});r2=true}catch{}try{await env.MATERIALIZE_QUEUE.metrics();queue=true}catch{}return {architecture:"CLOUDFLARE_BINDINGS",d1,r2,queue,internalKey:Boolean(env.CORVO_INTERNAL_KEY),signingKey:Boolean(env.CORVO_SIGNING_KEY),r2CredentialsStoredInD1:false};}
