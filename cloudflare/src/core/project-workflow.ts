...
export const PROJECT_WORKFLOW_TAGS=["READ","REFERENCE_ANALYSIS_WORKING","REFERENCE_CHECKED","COLLECTOR_WORKING","COLLECTOR_FINISHED","VISUAL_ANALYST_WORKING","VISUAL_ANALYST_FINISHED","DOWNLOADER_WORKING","DOWNLOADER_COMPLETED","QA_CONCLUIDO","THUMBS_WORKING","TITLES_WORKING"] as const;
...
export async function projectSlotSnapshot(env:Env,projectId:string){
  ...
  const [tags,script,referenceBrief,thumbs,titles,items,candidates,packages,slotAccess,production,visualSlotTags]=await Promise.all([
    ...
    env.DB.prepare("SELECT id,type,status,file_name,size_bytes,ready_at,downloaded_at,created_at,error,revision_hash FROM v2_download_packages WHERE project_id=? ORDER BY created_at DESC LIMIT 50").bind(projectId).all<Record<string,unknown>>(),
    ...
    env.DB.prepare(`SELECT COUNT(*) production_slots_total,
      SUM(CASE WHEN (asset_id IS NOT NULL OR candidate_id IS NOT NULL) AND status IN ('ASSIGNED_FOR_QA','RESOLVED','FROZEN','APPROVED','COMPLETED') THEN 1 ELSE 0 END) production_slots_resolved,
      SUM(CASE WHEN status='ASSIGNED_FOR_QA' AND (asset_id IS NOT NULL OR candidate_id IS NOT NULL) THEN 1 ELSE 0 END) production_slots_assigned_for_qa,
      SUM(CASE WHEN status='FROZEN' AND asset_id IS NOT NULL THEN 1 ELSE 0 END) production_slots_frozen,
      SUM(CASE WHEN status='RELINK_REQUIRED' THEN 1 ELSE 0 END) production_slots_relink_required,
      SUM(CASE WHEN status IN ('PENDING','UNRESOLVED') OR ((asset_id IS NULL AND candidate_id IS NULL) AND status NOT IN ('RELINK_REQUIRED')) THEN 1 ELSE 0 END) production_slots_pending,
      ...
      FROM v2_production_slots WHERE project_id=? AND status<>'RETIRED'`).bind(projectId,projectId,projectId).first<Record<string,unknown>>().catch(()=>null),
  ...
  const productionTotal=Number(production?.production_slots_total||0),productionResolved=Number(production?.production_slots_resolved||0),productionAssignedForQa=Number(production?.production_slots_assigned_for_qa||0),productionFrozen=Number(production?.production_slots_frozen||0),productionRelink=Number(production?.production_slots_relink_required||0),productionPending=Number(production?.production_slots_pending||0);
  ...
  const slots=[
    ...
    slot("candidates","Coleta / candidatas",productionTotal>0?(productionPending===0&&productionRelink===0?"READY":activeTags.some(t=>t.tag==="COLLECTOR_WORKING")?"WORKING":"WAITING"):(target>0&&materialized>=target?"READY":target>0?"WORKING":"WAITING"),productionTotal>0?`${Math.max(0,productionTotal-productionPending-productionRelink)}/${productionTotal} abastecidos · ${productionPending} sem imagem · ${productionRelink} relink`:(target?`${materialized}/${target} MATERIALIZED · ${Number(candidates?.reserve||0)} reserva`:"Sem cenas de coleta"),productionTotal>0?(productionTotal?Math.max(0,productionTotal-productionPending-productionRelink)/productionTotal*100:0):(target?materialized/target*100:0)),
...
export async function setProjectLifecycle(env:Env,input:{projectIds:string[];action:"COMPLETE"|"REJECT"|"REOPEN";reason?:string}){
  ...
  for(const projectId of ids){
    ...
    if(input.action==="COMPLETE"){
      const slots=await env.DB.prepare("SELECT COUNT(*) total,SUM(CASE WHEN asset_id IS NOT NULL AND status IN ('RESOLVED','FROZEN','APPROVED','COMPLETED') THEN 1 ELSE 0 END) resolved FROM v2_production_slots WHERE project_id=? AND status<>'RETIRED'").bind(projectId).first<Record<string,unknown>>().catch(()=>null);
      const total=Number(slots?.total||0),resolved=Number(slots?.resolved||0);
      const pkgRows=await env.DB.prepare("SELECT type,status FROM v2_download_packages WHERE project_id=? AND type IN ('PROJECT_IMAGES_ZIP','PROJECT_SCRIPT_TXT','PROJECT_PUBLICATION_ZIP') ORDER BY created_at DESC LIMIT 30").bind(projectId).all<Record<string,unknown>>().catch(()=>({results:[]} as unknown as D1Result<Record<string,unknown>>));
      const latestFinal=new Map<string,Record<string,unknown>>();for(const row of pkgRows.results||[]){const type=upper(row.type);if(!latestFinal.has(type))latestFinal.set(type,row);}const artifactReady=(type:string)=>["READY_FOR_DOWNLOAD","DOWNLOADED","COMPLETED"].includes(upper(latestFinal.get(type)?.status));const finalArtifactsReady={images:artifactReady("PROJECT_IMAGES_ZIP"),script:artifactReady("PROJECT_SCRIPT_TXT"),publication:artifactReady("PROJECT_PUBLICATION_ZIP")};const requiredArtifactsReady=finalArtifactsReady.images&&finalArtifactsReady.script;
      const [script,legacyRefs]=await Promise.all([
        env.DB.prepare("SELECT 1 ok FROM automatic_project_files WHERE project_id=? AND upper(role)='SCRIPT' LIMIT 1").bind(projectId).first().catch(()=>null),
        env.DB.prepare("SELECT COUNT(*) total,SUM(CASE WHEN upper(item_key) LIKE 'REF-%' THEN 1 ELSE 0 END) refs FROM automatic_project_items WHERE project_id=?").bind(projectId).first<Record<string,unknown>>().catch(()=>null),
      ]);
      const referenceOnly=Number(legacyRefs?.total||0)>0&&Number(legacyRefs?.refs||0)>=Number(legacyRefs?.total||0);
      if(total===0&&(script||referenceOnly)){results.push({projectId,error:"PRODUCTION_SLOTS_REQUIRED",production_slots_total:0,reason:referenceOnly?"REFERENCE_POOLS_ARE_NOT_FINAL_OUTPUT":"SCRIPT_REQUIRES_PRODUCTION_MODEL"});continue;}
      if(total>0&&(resolved<total||!requiredArtifactsReady)){results.push({projectId,error:"PRODUCTION_NOT_COMPLETE",production_slots_total:total,production_slots_resolved:resolved,required_artifacts_ready:requiredArtifactsReady,publication_optional:true,final_artifacts_ready:finalArtifactsReady});continue;}
    }
