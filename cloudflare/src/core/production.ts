...
import { finalizeProjectQa, materializeProductionModel, productionCompletionGate, reconcileLegacyProjectItemsFromProduction, type ProductionSceneSeed } from "./production-model";
...
async function zipIndexEntries(env:Env,key:string,size:number){
  ...
  const tailLength=Math.min(size,65557),tailOffset=Math.max(0,size-tailLength),tailObj=await getRange(tailOffset,tailLength);if(!tailObj)return[];const tail=new Uint8Array(await tailObj.arrayBuffer());let eocd=-1;for(let i=tail.length-22;i>=0;i--){if(tail[i]===0x50&&tail[i+1]===0x4b&&tail[i+2]===0x05&&tail[i+3]===0x06){eocd=i;break;}}if(eocd<0)throw new Error("ZIP_INDEX_EOCD_NOT_FOUND");const tailView=new DataView(tail.buffer,tail.byteOffset,tail.byteLength),count=tailView.getUint16(eocd+10,true),centralSize=tailView.getUint32(eocd+12,true),centralOffset=tailView.getUint32(eocd+16,true);const centralObj=await getRange(centralOffset,centralSize);if(!centralObj)throw new Error("ZIP_INDEX_CENTRAL_DIRECTORY_MISSING");const bytes=new Uint8Array(await centralObj.arrayBuffer()),view=new DataView(bytes.buffer,bytes.byteOffset,bytes.byteLength),decoder=new TextDecoder();const entries:Array<{name:string;method:number;compressedSize:number;uncompressedSize:number;localOffset:number}>=[];let cursor=0;for(let i=0;i<count;i++){if(cursor+46>bytes.length||view.getUint32(cursor,true)!==0x02014b50)throw new Error("ZIP_INDEX_INVALID_CENTRAL_DIRECTORY");const method=view.getUint16(cursor+10,true),compressedSize=view.getUint32(cursor+20,true),uncompressedSize=view.getUint32(cursor+24,true),nameLen=view.getUint16(cursor+28,true),extra=view.getUint16(cursor+30,true),comment=view.getUint16(cursor+32,true),localOffset=view.getUint32(cursor+42,true),name=decoder.decode(bytes.slice(cursor+46,cursor+46+nameLen));entries.push({name,method,compressedSize,uncompressedSize,localOffset});cursor+=46+nameLen+extra+comment;}return entries;
}
async function zipIndexNames(env:Env,key:string,size:number){return (await zipIndexEntries(env,key,size)).map(entry=>entry.name);}
function sniffImageBytes(b:Uint8Array){if(b.length>=3&&b[0]===0xff&&b[1]===0xd8&&b[2]===0xff)return"image/jpeg";if(b.length>=8&&b[0]===0x89&&b[1]===0x50&&b[2]===0x4e&&b[3]===0x47&&b[4]===0x0d&&b[5]===0x0a&&b[6]===0x1a&&b[7]===0x0a)return"image/png";if(b.length>=12&&String.fromCharCode(...b.slice(0,4))==="RIFF"&&String.fromCharCode(...b.slice(8,12))==="WEBP")return"image/webp";return"";}
async function validateImagesZipObject(env:Env,input:{projectId:string;r2Key:string;sizeBytes:number;expectedNames:string[];packageId?:string}){
  const entries=await zipIndexEntries(env,input.r2Key,input.sizeBytes),indexed=entries.map(entry=>entry.name),expected=input.expectedNames.map(safeName),expectedSet=new Set(expected),foundSet=new Set(indexed),missing=expected.filter(name=>!foundSet.has(name)),unexpected=indexed.filter(name=>!expectedSet.has(name)),duplicates=[...new Set(indexed.filter((name,i)=>indexed.indexOf(name)!==i))],flat=indexed.every(name=>Boolean(name)&&!/[\/]/.test(name)&&!name.endsWith("/"));
  const invalidDetails:Array<{name:string;reason:string;expected_mime?:string;actual_mime?:string}>=[];const getRange=(offset:number,length:number)=>(env.MEDIA as unknown as {get:(key:string,opts?:unknown)=>Promise<R2ObjectBody|null>}).get(input.r2Key,{range:{offset,length}});
  for(const entry of entries){if(entry.method!==0){invalidDetails.push({name:entry.name,reason:"UNSUPPORTED_COMPRESSION_METHOD"});continue;}const expectedMime=expectedMimeForTarget(entry.name);if(!expectedMime){invalidDetails.push({name:entry.name,reason:"INVALID_IMAGE_EXTENSION"});continue;}const headerObj=await getRange(entry.localOffset,30);if(!headerObj){invalidDetails.push({name:entry.name,reason:"LOCAL_HEADER_MISSING"});continue;}const h=new Uint8Array(await headerObj.arrayBuffer());if(h.length<30||new DataView(h.buffer,h.byteOffset,h.byteLength).getUint32(0,true)!==0x04034b50){invalidDetails.push({name:entry.name,reason:"LOCAL_HEADER_INVALID"});continue;}const hv=new DataView(h.buffer,h.byteOffset,h.byteLength),nameLen=hv.getUint16(26,true),extraLen=hv.getUint16(28,true),payloadOffset=entry.localOffset+30+nameLen+extraLen,payload=await getRange(payloadOffset,Math.min(32,entry.uncompressedSize));if(!payload){invalidDetails.push({name:entry.name,reason:"IMAGE_PAYLOAD_MISSING"});continue;}const actualMime=sniffImageBytes(new Uint8Array(await payload.arrayBuffer()));if(actualMime!==expectedMime)invalidDetails.push({name:entry.name,reason:"IMAGE_FORMAT_MISMATCH",expected_mime:expectedMime,actual_mime:actualMime||"unknown"});}
  const report={project_id:input.projectId,package_id:input.packageId||null,expected:expected.length,found:indexed.length,missing:missing.length,unexpected:unexpected.length,duplicates:duplicates.length,invalid:invalidDetails.length,flat,ok:expected.length===indexed.length&&missing.length===0&&unexpected.length===0&&duplicates.length===0&&invalidDetails.length===0&&flat,missing_names:missing.slice(0,50),unexpected_names:unexpected.slice(0,50),duplicate_names:duplicates.slice(0,50),invalid_entries:invalidDetails.slice(0,50),r2_key:input.r2Key,size_bytes:input.sizeBytes};return report;
}

export async function validateProjectImagesZip(env:Env,input:{projectId:string;packageId?:string}){
  const packageRow=input.packageId?await env.DB.prepare("SELECT * FROM v2_download_packages WHERE id=? AND project_id=? AND type='PROJECT_IMAGES_ZIP'").bind(input.packageId,input.projectId).first<Record<string,unknown>>():await env.DB.prepare("SELECT * FROM v2_download_packages WHERE project_id=? AND type='PROJECT_IMAGES_ZIP' AND r2_key IS NOT NULL ORDER BY created_at DESC LIMIT 1").bind(input.projectId).first<Record<string,unknown>>();
  if(!packageRow)return{error:"IMAGES_ZIP_NOT_FOUND",status:404,project_id:input.projectId}as const;const r2Key=clean(packageRow.r2_key);if(!r2Key)return{error:"IMAGES_ZIP_R2_KEY_MISSING",status:409,project_id:input.projectId,package_id:packageRow.id}as const;const head=await env.MEDIA.head(r2Key);if(!head)return{error:"IMAGES_ZIP_R2_OBJECT_MISSING",status:409,project_id:input.projectId,package_id:packageRow.id,r2_key:r2Key}as const;
  const script=await activeScript(env,input.projectId),expectedNames=scriptImageRefs(script.text).map(safeName),report=await validateImagesZipObject(env,{projectId:input.projectId,r2Key,sizeBytes:Number(head.size),expectedNames,packageId:clean(packageRow.id)});const ts=nowMs(),eventId=await stableId("PEV",`${input.projectId}\n${packageRow.id}\nFINAL_IMAGES_ZIP_MANIFEST_VALIDATED\n${report.ok}`,12);await env.DB.prepare("INSERT OR IGNORE INTO automatic_project_events(id,project_id,event,status,detail,created_at) VALUES (?,?,'FINAL_IMAGES_ZIP_MANIFEST_VALIDATED',?,?,?)").bind(eventId,input.projectId,report.ok?"PASS":"FAIL",JSON.stringify(report),ts).run().catch(()=>undefined);return report;
}
...
export async function processPackageJob(env:Env,job:PackageJob){
  ...
  try{
    ...
    if(type==="PROJECT_SCRIPT_TXT"){
      ...
      await env.DB.prepare("INSERT INTO automatic_project_events(id,project_id,event,status,detail,created_at) VALUES (?,?,?,?,?,?)").bind(id("PEV"),job.projectId,"FINAL_IMAGES_ZIP_R2_VERIFIED","PROCESSING",JSON.stringify({packageId:job.packageId,expectedSize:materialized.expectedSize,actualSize,r2Key}),nowMs()).run().catch(()=>undefined);const manifestValidation=await validateImagesZipObject(env,{projectId:job.projectId,r2Key,sizeBytes:actualSize,expectedNames:bundle.expectedNames,packageId:job.packageId});await env.DB.prepare("INSERT INTO automatic_project_events(id,project_id,event,status,detail,created_at) VALUES (?,?,?,?,?,?)").bind(id("PEV"),job.projectId,"FINAL_IMAGES_ZIP_MANIFEST_VALIDATED",manifestValidation.ok?"PASS":"FAIL",JSON.stringify(manifestValidation),nowMs()).run().catch(()=>undefined);if(!manifestValidation.ok)throw new Error(`FORMA_ZIP_MANIFEST_INVALID:expected=${manifestValidation.expected}:found=${manifestValidation.found}:missing=${manifestValidation.missing}:unexpected=${manifestValidation.unexpected}:duplicates=${manifestValidation.duplicates}:invalid=${manifestValidation.invalid}:flat=${manifestValidation.flat}`);resultMeta={...resultMeta,images:manifestValidation.expected,productionSlots:bundle.slotsTotal,convertedFormats:bundle.converted,uploadMode:"KNOWN_LENGTH_UINT8ARRAY_NO_PIPETO",pipeToUsed:false,flat:manifestValidation.flat,expected:manifestValidation.expected,found:manifestValidation.found,missing:manifestValidation.missing,unexpected:manifestValidation.unexpected,duplicateNames:manifestValidation.duplicates,invalidFormat:manifestValidation.invalid,manifestValidated:true};
    ...
    if(type==="PROJECT_IMAGES_ZIP")statements.push(env.DB.prepare("UPDATE automatic_projects SET zip_r2_key=?,zip_file_name=?,zip_size_bytes=?,status='ACTIVE',pipeline_status='READY_FOR_DOWNLOAD',next_action='DOWNLOAD_IMAGES_ZIP',workflow_updated_at=? WHERE id=?").bind(r2Key,fileName,actualSize,done,job.projectId));
    await env.DB.batch(statements);await updateProjectWorkflow(env,{projectId:job.projectId,activate:type==="PROJECT_IMAGES_ZIP"?["QA_CONCLUIDO"]:[],clear:["DOWNLOADER_WORKING"],metadata:{packageId:job.packageId,type,ready:true,manifestValidated:type==="PROJECT_IMAGES_ZIP"?true:undefined}}).catch(()=>undefined);await reconcileAutomaticProject(env,job.projectId).catch(()=>undefined);return{packageId:job.packageId,type,status:"READY_FOR_DOWNLOAD",r2Key,fileName,sizeBytes:actualSize,...resultMeta};
  }catch(error){
...
export async function finalizeQaAndQueueDelivery(env:Env,input:{projectId:string;operationId?:string;finalizedBy?:string}){
  const qa=await finalizeProjectQa(env,input);if((qa as any)?.error)return qa;
  const legacy_reconciliation=await reconcileLegacyProjectItemsFromProduction(env,input.projectId).catch(error=>({error:error instanceof Error?error.message:String(error),changed:0}));
  const [gate,tags]=await Promise.all([productionCompletionGate(env,input.projectId),listProjectTagsFlat(env,input.projectId).catch(()=>[] as Record<string,unknown>[])]),blockingTags=tags.filter((tag:any)=>["REVISAR","REVISADO_PARA_QA"].includes(clean(tag.tag_key||tag.tag).toUpperCase())).map((tag:any)=>({slot_id:tag.slot_id||null,tag_key:clean(tag.tag_key||tag.tag).toUpperCase()}));
  const qaClosed=Boolean(gate.complete)&&blockingTags.length===0;if(!qaClosed)return {...(qa as any),legacy_reconciliation,project_qa_closed:false,qa_blockers:{production_slots_relink_required:gate.production_slots_relink_required,production_slots_assigned_for_qa:gate.production_slots_assigned_for_qa,production_slots_pending:gate.production_slots_pending,tags:blockingTags},delivery_queued:false,next_action:Number(gate.production_slots_relink_required||0)>0?"RELINK_PRODUCTION_SLOTS":Number(gate.production_slots_assigned_for_qa||0)>0?"FINALIZE_QA_OR_REJECT_SLOTS":"ASSIGN_ASSETS_TO_SLOTS"};
  const ts=nowMs();await env.DB.prepare("UPDATE automatic_projects SET status='ACTIVE',pipeline_status='QA_CONCLUIDO',next_action='GENERATE_AND_VALIDATE_IMAGES_ZIP',state_version=state_version+1,workflow_updated_at=?,updated_at=? WHERE id=?").bind(ts,ts,input.projectId).run();await updateProjectWorkflow(env,{projectId:input.projectId,activate:["QA_CONCLUIDO"],clear:["VISUAL_ANALYST_WORKING"],ownerId:input.finalizedBy||"MCP_QA",executionId:input.operationId,metadata:{qaClosed:true,productionSlots:gate.production_slots_total}}).catch(()=>undefined);
  const queueOperationId=await stableId("OP",`${input.projectId}\nPROJECT_IMAGES_ZIP\n${clean((qa as any).operation_id)||clean(input.operationId)||"QA_CLOSED"}`,12),delivery=await queueFinalPackage(env,{projectId:input.projectId,type:"PROJECT_IMAGES_ZIP",operationId:queueOperationId});
  const ready=["READY_FOR_DOWNLOAD","DOWNLOADED","COMPLETED"].includes(clean((delivery as any).status).toUpperCase());const validation=ready?await validateProjectImagesZip(env,{projectId:input.projectId,packageId:clean((delivery as any).id||(delivery as any).packageId)||undefined}).catch(error=>({ok:false,error:error instanceof Error?error.message:String(error)})):null;
  return {...(qa as any),legacy_reconciliation,project_qa_closed:true,qa_blockers:{production_slots_relink_required:0,production_slots_assigned_for_qa:0,production_slots_pending:0,tags:[]},delivery_queued:true,images_zip:delivery,manifest_validation:validation,next_action:ready&&Boolean((validation as any)?.ok)?"DOWNLOAD_IMAGES_ZIP":"WAIT_FOR_IMAGES_ZIP_WORKER"};
}
