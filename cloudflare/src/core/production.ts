...
export async function finalizeQaAndQueueDelivery(env:Env,input:{projectId:string;operationId?:string;finalizedBy?:string}){
  ...
  const nextAction=clean((qa as any).next_action);
  return {...(qa as any),legacy_reconciliation,delivery_queued:false,legacy_images_zip_handoff:false,quiz_handoff:nextAction==="QUIZ_RENDER",next_action:nextAction||"ASSIGN_ASSETS_TO_SLOTS"};
}
