...
function createServer(env: Env, request: Request) {
  const server = new McpServer({ name: "corvo-library-v2", version: "0.20.63" });
  ...
  server.registerTool("verificar_saude", {
    ...
    return output({ ok:d1Ok&&r2Ok, architecture:"D1_R2_QUEUE", version:"0.20.63", core_version:"0.20.63", d1:d1Ok?"ok":"error", r2:r2Ok?"ok":"error", schema_contract_version:"2.29.0" });
  });
  server.registerTool("obter_versao_core", {
    ...
    inputSchema: {},
  }, async () => output({ok:true,service:"corvo-core",version:"0.20.63",core_version:"0.20.63",schema_contract_version:"2.29.0",d1_read_required:false}));
