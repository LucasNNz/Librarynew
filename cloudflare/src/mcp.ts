...
function createServer(env: Env, request: Request) {
  const server = new McpServer({ name: "corvo-library-v2", version: "0.20.64" });
  ...
  server.registerTool("verificar_saude", {
    ...
    return output({ ok:d1Ok&&r2Ok, architecture:"D1_R2_QUEUE", version:"0.20.64", core_version:"0.20.64", d1:d1Ok?"ok":"error", r2:r2Ok?"ok":"error", schema_contract_version:"2.29.0" });
  });
  server.registerTool("obter_versao_core", {
    ...
    inputSchema: {},
  }, async () => output({ok:true,service:"corvo-core",version:"0.20.64",core_version:"0.20.64",schema_contract_version:"2.29.0",d1_read_required:false}));
  ...
  server.registerTool("anexar_thumb_arquivo", {
    description:"ROTA DIRETA PARA ARQUIVO LOCAL/ANEXO DO CHATGPT: recebe o file object no próprio argumento, lê bytes/base64 ou referência HTTPS reescrita pelo runtime, grava internamente no R2, cria candidate MATERIALIZED e anexa como THUMB em uma única chamada. Não devolve ticket PREPARED e não exige PUT externo nem URL pública fornecida pelo usuário.",
    inputSchema:{ projeto_id:z.string().min(1), arquivo:z.unknown().describe("File object/anexo local do ChatGPT com bytes/base64 ou referência de download reescrita pelo runtime"), nome:z.string().optional(), agente:z.string().optional() },
  }, async(v)=>output(await attachProjectThumbFromFileObject(env,{projectId:v.projeto_id,file:v.arquivo,name:v.nome,agentOrigin:v.agente||"MCP_THUMB_FILE"})));
  ...
  server.registerTool("atualizar_estados_projeto", {
    description: "Coordena tags simultâneas do projeto. Permite ativar/remover READ, REFERENCE_ANALYSIS_WORKING, REFERENCE_CHECKED, COLLECTOR_WORKING, COLLECTOR_FINISHED, VISUAL_ANALYST_WORKING, VISUAL_ANALYST_FINISHED, QA_CONCLUIDO, DOWNLOADER_WORKING, DOWNLOADER_COMPLETED, THUMBS_WORKING e TITLES_WORKING. Tags WORKING usam heartbeat/TTL e expiram para o passo estável anterior.",
  ...
  server.registerTool("validar_imagens_zip_projeto", {
    description:"Gate interno do pacote final: inspeciona o índice e os bytes de imagens.zip no R2 e devolve expected/found/missing/unexpected/duplicates/invalid/flat. READY_FOR_DOWNLOAD só é emitido pelo gerador após este mesmo gate passar.",
    inputSchema:{ projeto_id:z.string().min(1), pacote_id:z.string().optional() },
  }, async(v)=>output(await validateProjectImagesZip(env,{projectId:v.projeto_id,packageId:v.pacote_id})));
