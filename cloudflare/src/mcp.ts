...
function createServer(env: Env, request: Request) {
  const server = new McpServer({ name: "corvo-library-v2", version: "0.20.3" });
  ...
  server.registerTool("processar_importacao_zip", { description:"Processa/reprocessa idempotentemente ZIP recebido no R2, lê IMPORTACAO.txt, extrai mídia e cataloga no D1.", inputSchema:{ importacao_id:z.string().min(1) } }, async(v)=>output(await processZipImport(env,v.importacao_id)));
