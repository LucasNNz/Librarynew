...
async function health(env: Env) {
  ...
  return { ok: d1 === "ok" && r2 === "ok" && schema === "ok" && signing === "ok" && appAuth === "ok", service: "corvo-core", version: "0.20.39", d1, r2, schema, schemaContract, queue: "ok" as const, signing, appAuth, control, queueBacklog, infrastructure: { initialized: infrastructure.initialized, profile: infrastructure.profile } };
}

const FAST_READ_CACHE_NAMESPACE = "0.20.39";
...
async function fastReadJson(request:Request,ctx:ExecutionContext,ttlSeconds:number,producer:()=>Promise<unknown>) {
  ...
  // read by the running Core release so an older snapshot can never satisfy a 0.20.39
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/ui/boot" && request.method === "GET") {
      response=await fastReadJson(request,ctx,12,async()=>{
        ...
        return {ok:true,version:"0.20.39",health:{app:"ok",architecture:"CLOUDFLARE_CORE",coreConfigured:true,core:coreHealth},...overview};
      });
