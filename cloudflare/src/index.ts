import { authorizeQuizExecutor, quizRpc, rendererStatus, uploadQuizPart, uploadQuizMedia, serveQuizMedia, QuizError } from "./quiz/service";
import { processQuizBrowserJob } from "./quiz/browser-executor";
import type { CorvoQueueJob, Env, MaterializeJob } from "./types";
import { handleMcp } from "./mcp";
import { authorized, createBrowserDeviceToken, validControlPairingRequest } from "./core/auth";
...
import { configureAutomaticProject, createAutomaticProject, getAutomaticProjectDetails, getProjectBlockers, getProjectSlot, getShortOperationalSnapshot, listAutomaticProjects, processAutomaticProject, projectAvailability, projectLog, reconcileAutomaticProject, reopenAutomaticProject, validateProjectConsistency } from "./core/projects";
...
import { publicMediaResponse } from "./core/public-media";
...
async function health(env: Env) {
  ...
  const renderer=await rendererStatus(env).catch(()=>({online:false,mode:'UNAVAILABLE',browser_closed_ok:false}));
  return { ok: d1 === "ok" && r2 === "ok" && schema === "ok" && signing === "ok" && appAuth === "ok", service: "corvo-core", version: "0.20.63", core_version: "0.20.63", d1, r2, schema, schemaContract, queue: "ok" as const, signing, appAuth, control, queueBacklog, renderer_online:renderer.online, renderer, infrastructure: { initialized: infrastructure.initialized, profile: infrastructure.profile } };
}

const FAST_READ_CACHE_NAMESPACE = "0.20.63";
...
async function fastReadJson(request:Request,ctx:ExecutionContext,ttlSeconds:number,producer:()=>Promise<unknown>) {
  ...
  // read by the running Core release so an older snapshot can never satisfy a 0.20.63
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname.startsWith("/files/") && signedMediaMethod) {
      return withCors(await publicMediaResponse(request,"asset-file","0.20.63",()=>serveFile(request, decodeURIComponent(url.pathname.slice(7)), env)), request);
    }
    if (url.pathname.startsWith("/thumbs/") && signedMediaMethod) {
      return withCors(await publicMediaResponse(request,"asset-thumb","0.20.63",()=>serveAssetThumbnail(request, decodeURIComponent(url.pathname.slice(8)), env)), request);
    }
    if (url.pathname.startsWith("/candidate-files/") && signedMediaMethod) {
      return withCors(await publicMediaResponse(request,"candidate-file","0.20.63",()=>serveCandidateFile(request, decodeURIComponent(url.pathname.slice(17)), env)), request);
    }
    if (url.pathname.startsWith("/supervisor-candidate-files/") && signedMediaMethod) {
      return withCors(await publicMediaResponse(request,"supervisor-candidate-file","0.20.63",()=>serveSupervisorCandidateFile(request, decodeURIComponent(url.pathname.slice(28)), env)), request);
    }
    if (url.pathname.startsWith("/package-files/") && signedMediaMethod) return withCors(await publicMediaResponse(request,"package-file","0.20.63",()=>servePackageFile(request,decodeURIComponent(url.pathname.slice(15)),env)),request);
    if (url.pathname.startsWith("/project-media/") && signedMediaMethod) return withCors(await publicMediaResponse(request,"project-media","0.20.63",()=>serveProjectMedia(request,decodeURIComponent(url.pathname.slice(15)),env)),request);
    if (url.pathname.startsWith("/project-files/") && signedMediaMethod) return withCors(await publicMediaResponse(request,"project-file","0.20.63",()=>serveProjectFile(request,decodeURIComponent(url.pathname.slice(15)),env)),request);
    ...
    // D1-free public control-plane probes. These must remain reachable even when
    ...
    if (url.pathname === "/version" && request.method === "GET") {
      return withCors(json({ok:true,service:"corvo-core",version:"0.20.63",core_version:"0.20.63",schema_contract_version:"2.29.0",d1_read_required:false,auth_mode:"DEVICE_TOKEN_V1"}),request);
    }
    if (url.pathname === "/control/pair-browser" && request.method === "POST") {
      ...
      return withCors(json({ok:true,...session,core_version:"0.20.63",d1_read_required:false}),request);
    }

    if (url.pathname.startsWith("/quiz/media/") && signedMediaMethod) return withCors(await serveQuizMedia(env,request,url.pathname.split("/").pop()!),request);

    // Ephemeral headless-renderer bridge. Authorization is the active D1 job
    // lease; it exposes only asset lookup and artifact upload operations.
    if (url.pathname === "/quiz/executor/rpc" && request.method === "POST") {
      if(!await authorizeQuizExecutor(env,request))return withCors(json({ok:false,error:"QUIZ_EXECUTOR_UNAUTHORIZED"},{status:401}),request);
      try{const body=await request.json() as any;if(!['asset.resolve','upload-start','upload-finish','upload-abort'].includes(body.op))throw new QuizError('EXECUTOR_OPERATION_NOT_ALLOWED',403);return withCors(json(await quizRpc(env,request,body.op,body.payload||{})),request);}
      catch(e){return withCors(json({ok:false,error:e instanceof Error?e.message:'QUIZ_ERROR'},{status:e instanceof QuizError?e.status:500}),request);}
    }
    const executorPart=/^\/quiz\/executor\/upload\/([\w-]+)\/(\d+)$/.exec(url.pathname);
    if(executorPart&&request.method==='PUT'){
      if(!await authorizeQuizExecutor(env,request))return withCors(json({ok:false,error:'QUIZ_EXECUTOR_UNAUTHORIZED'},{status:401}),request);
      try{return withCors(json(await uploadQuizPart(env,request,executorPart[1],Number(executorPart[2]))),request);}catch(e){return withCors(json({ok:false,error:e instanceof Error?e.message:'QUIZ_ERROR'},{status:e instanceof QuizError?e.status:500}),request);}
    }
    ...
    if (url.pathname === "/quiz/rpc" && request.method === "POST") {
      try { const body = await request.json() as any; return withCors(json(await quizRpc(env,request,body.op,body.payload||{})),request); }
      catch(e){return withCors(json({ok:false,error:e instanceof Error?e.message:"QUIZ_ERROR"},{status:e instanceof QuizError?e.status:500}),request);}
    }
    const quizPart = /^\/quiz\/upload\/([\w-]+)\/(\d+)$/.exec(url.pathname);
    if(quizPart && request.method === "PUT") {
      try {return withCors(json(await uploadQuizPart(env,request,quizPart[1],Number(quizPart[2]))),request);}
      catch(e){return withCors(json({ok:false,error:e instanceof Error?e.message:"QUIZ_ERROR"},{status:e instanceof QuizError?e.status:500}),request);}
    }
    if (url.pathname === "/quiz/media" && request.method === "POST") {
      try { return withCors(json(await uploadQuizMedia(env,request)),request); }
      catch(e){return withCors(json({ok:false,error:e instanceof Error?e.message:"QUIZ_ERROR"},{status:e instanceof QuizError?e.status:500}),request);}
    }
    ...
    if (url.pathname === "/ui/boot" && request.method === "GET") {
      response=await fastReadJson(request,ctx,12,async()=>{
        ...
        return {ok:true,version:"0.20.63",health:{app:"ok",architecture:"CLOUDFLARE_CORE",coreConfigured:true,core:coreHealth},...overview};
      });
    ...
    else if (/^\/projects\/[^/]+\/short$/.test(url.pathname) && request.method === "GET") {
      const projectId=decodeURIComponent(url.pathname.split("/")[2]); const sinceRaw=url.searchParams.get("sinceVersion"); const sinceVersion=sinceRaw==null?undefined:Number(sinceRaw); const value=await getShortOperationalSnapshot(env,projectId,Number.isFinite(sinceVersion as number)?sinceVersion:undefined); response=value?json(value):json({error:"NOT_FOUND"},{status:404});
    }
    else if (/^\/projects\/[^/]+\/blockers$/.test(url.pathname) && request.method === "GET") {
      const projectId=decodeURIComponent(url.pathname.split("/")[2]); const value=await getProjectBlockers(env,projectId); response=value?json(value):json({error:"NOT_FOUND"},{status:404});
    }
  ...
  async queue(batch: MessageBatch<CorvoQueueJob>, env: Env) {
    ...
    await Promise.all(batch.messages.map(async message => {
      ...
      try {
        ...
        else if (job.kind === "QUIZ_JOB") { const result=await processQuizBrowserJob(env,job);if(result&&'retry' in result&&result.retry)await env.MATERIALIZE_QUEUE.send(job,{delaySeconds:5});message.ack(); }
