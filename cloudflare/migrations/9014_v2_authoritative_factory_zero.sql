-- CORVO LIBRARY V2 2.14.0 — authoritative factory zero, compatible with Core 0.19+
-- Runs through the existing /control/apply-migrations route. No Worker self-update is
-- required. This migration is one-shot because v2_migrations_applied records it.
-- Preserve only structural schema/migration data and persistent D1/R2 infrastructure config.
...
-- These are the only persisted configuration tables intentionally retained:
...
VALUES ('schema_version','2.14.0',unixepoch('now')*1000);
INSERT OR REPLACE INTO v2_schema_meta(key,value,updated_at)
...
INSERT OR REPLACE INTO v2_schema_meta(key,value,updated_at)
VALUES ('factory_zero_release_0_20_5','DONE',unixepoch('now')*1000);
INSERT OR REPLACE INTO v2_schema_meta(key,value,updated_at)
VALUES ('factory_zero_release_0_20_6','DONE',unixepoch('now')*1000);
