...
/**
 * Update one Worker secret without touching the script or any other binding.
 * Used by browser recovery to refresh only the control credential supplied by
 * the administrator; app/session/signing keys remain untouched.
 */
export async function putWorkerSecretWithToken(token: string, accountId: string, workerName: string, name: string, text: string) {
  await cfApi<unknown>(token, `/accounts/${encodeURIComponent(accountId)}/workers/scripts/${encodeURIComponent(workerName)}/secrets`, {
    method: "PUT",
...
/**
 * Publish a new Core bundle while inheriting the current Worker's bindings.
 * This is the safe path for recovering an existing installation: D1/R2/Queue,
 * plain-text configuration and secret_text bindings are preserved from the
 * previous upload instead of being regenerated by a second browser.
 */
export async function updateWorkerBundlePreservingBindings(token: string, accountId: string, workerName: string, workerBundle: string) {
  const metadata = {
    main_module: "corvo-core-v2.mjs",
    compatibility_date: "2026-08-29",
  ...
  const form = new FormData();
  form.append("metadata", new Blob([JSON.stringify(metadata)], { type: "application/json" }), "metadata.json");
  form.append("corvo-core-v2.mjs", new Blob([workerBundle], { type: "application/javascript+module" }), "corvo-core-v2.mjs");
  await cfApi<unknown>(token, `/accounts/${encodeURIComponent(accountId)}/workers/scripts/${encodeURIComponent(workerName)}`, { method: "PUT", body: form });
}

export async function workerExists(token: string, accountId: string, workerName: string) {
  try {
    await cfApi<unknown[]>(token, `/accounts/${encodeURIComponent(accountId)}/workers/scripts/${encodeURIComponent(workerName)}/secrets`);
    return true;
  } catch (error) {
...
export async function workerCoreUrl(token: string, accountId: string, workerName: string) {
  // Keep the stable workers.dev route enabled; this is idempotent.
  await cfApi<unknown>(token, `/accounts/${encodeURIComponent(accountId)}/workers/scripts/${encodeURIComponent(workerName)}/subdomain`, {
    method: "POST",
    body: JSON.stringify({ enabled: true, previews_enabled: false }),
  });
  const accountSubdomain = await cfApi<{ subdomain: string }>(token, `/accounts/${encodeURIComponent(accountId)}/workers/subdomain`);
  return `https://${workerName}.${accountSubdomain.subdomain}.workers.dev`;
}
