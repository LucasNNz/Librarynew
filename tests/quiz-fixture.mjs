...
const queueMessages=[];
const env={DB,MEDIA,MATERIALIZE_QUEUE:{async send(message){queueMessages.push(message)}},BROWSER:{async fetch(){return new Response(JSON.stringify({maxConcurrentSessions:6}))}}},request=new Request('https://core.example/quiz/rpc');
const rpc=(op,p={})=>quizRpc(env,request,op,p);
const project={version:4,scenes:[{title:'Manual',format:'16:9'}],active_scene:1,audio:null};


export {env,rpc,quizRpc,serveQuizMedia,uploadQuizMedia,uploadQuizPart,project,sqlite,queueMessages};
