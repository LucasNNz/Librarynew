...
export async function GET(request:NextRequest,context:{params:Promise<{id:string}>}){
  ...
  const {id}=await context.params;const since=request.nextUrl.searchParams.get("sinceVersion");
  const query=since!=null?`?sinceVersion=${encodeURIComponent(since)}`:"";
  const r=await coreFetch(`/projects/${encodeURIComponent(id)}/short${query}`);
